"""
EDUPREDICT – Final Training Code (v2)
======================================
...
Input file
----------
  final_dataset2.csv – 856-record dataset with realistic label noise and
                       an actual attendance_rate column, targeting ~86%
                       model accuracy.
...
# 0. Paths
# ---------------------------------------------------------------------------
DATASET_CSV = "final_dataset2.csv"
...
def load_subject_data(path: str) -> pd.DataFrame:
    """Load final_dataset2.csv (task scores, prelim/midterm grades, exam data,
    and actual attendance_rate)."""
    df = pd.read_csv(path)
    ...
    # attendance_rate is present in final_dataset2.csv – use it directly
    if "attendance_rate" not in df.columns:
        df["attendance_rate"] = 75.0  # fallback if column is missing
...
def main():
    ...
    print(f"  final_dataset2.csv : {len(df)} rows")
    print(f"\n  Risk distribution:")
